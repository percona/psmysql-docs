...
2. Use the `percona-release` tool to setup the repository for Percona Server for MySQL 8.4.
...
3. Enable the `{{pkg}} release` repository.
    ...
    ??? example "Expected output"
        ...
        ```{.text .no-copy}
        * Enabling the Percona Server 8.4 repository
        <*> All done!
        ```

4. Install the latest version of Percona Server for MySQL 8.4. This installation may take some time.
    ...
    ??? example "Expected output"
        ...
        ```{.text .no-copy}
        Percona Server 8.4 release/x86_64 YUM repository                                             1.0 MB/s | 2.3 MB     00:02
