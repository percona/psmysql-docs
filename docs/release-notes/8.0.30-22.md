...
* [SASL-based LDAP variables](../ldap-sasl-variables.md)

* [Fallback server variables for simple LDAP and SASL-based LDAP](../ldap-sasl-variables.md)
...
* [PS-6002](https://jira.percona.com/browse/PS-6002): Added the global variable [`--replica-enable-event`](../group-replication-system-variables.md) to maintain the create/alter event state on replicas.
  
* [PS-7980](https://jira.percona.com/browse/PS-7980): Added the global system variables [`--authentication_ldap_simple_group_role_mapping`](../ldap-sasl-variables.md) and [`--authentication_ldap_sasl_group_role_mapping`](../ldap-sasl-variables.md). When a user logs in with LDAP authentication, the server checks if the LDAP user is a member of any group specified in this variable. If the check is successful, the matching MySQL roles are automatically granted to the user.
  
* [PS-8275](https://jira.percona.com/browse/PS-8275): Implements the ability to eject a cluster node when the node exceeds the flow control threshold by adding a `MAJORITY` mode to [`group_replication_flow_control_mode`](../group-replication-system-variables.md) and a global system variable [`group_replication_auto_evict_timeout`](../group-replication-system-variables.md).
...
* [PS-8155](https://jira.percona.com/browse/PS-8155): Implements support for multiple LDAP server for the simple LDAP authentication plugin and the SASL-based LDAP authentication plugin. If the appropriate system variable is set, and if the primary server is unavailable, the authentication plugin attempts to connect and authenticate using a fallback server. A user can also specify multiple fallback servers. The following global system variables are:
  ...
  * [`authentication_ldap_simple_fallback_server_host`](../ldap-sasl-variables.md)
  
  * [`authentication_ldap_simple_fallback_server_port`](../ldap-sasl-variables.md)
  
  * [`authentication_ldap_sasl_fallback_server_host`](../ldap-sasl-variables.md)
  
  * [`authentication_ldap_sasl_fallback_server_port`](../ldap-sasl-variables.md)
