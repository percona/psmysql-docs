...
Percona Server for MySQL 8.0.37 deprecated `SEQUENCE_TABLE()`, and this function can be removed in a future release. We recommend that you use [`PERCONA_SEQUENCE_TABLE()`](percona-sequence-table.md) instead.
...
As with any [derived tables :octicons-link-external-16:](https://dev.mysql.com/doc/refman/8.0/en/derived-tables.html), a table function requires an [alias :octicons-link-external-16:](https://dev.mysql.com/doc/refman/8.0/en/identifiers.html) in the `SELECT` statement.
