...
2. Select a product, for example, Percona Server for MySQL
3. In `Product Group`, select which product, for example, Percona Server {{vers}}
4. In `Version`, select the version, for example, PERCONA-SERVER-{{release}}
5. In `Platform`, select the operating system, for example, DEBIAN GNU/LINUX 12.0 ("BOOKWORM").
