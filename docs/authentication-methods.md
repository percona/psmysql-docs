...
| OpenID Connect Authentication    | Authenticates users with signed JSON Web Tokens (JWTs) from an external IDP. Supported providers include Keycloak, Okta, and Microsoft Entra ID. Requires TLS, a socket, or shared memory. Supports group-to-role mapping and group-based proxy users. |
