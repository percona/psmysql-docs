...
Without bulk load, MyRocks keeps every change for the open transaction in memory. A large conversion can use all host memory. The Linux out-of-memory (OOM) killer is a kernel process that stops other processes to free memory. The OOM killer can stop `mysqld`.

To convert large MyISAM or InnoDB tables with `ALTER` or `INSERT INTO SELECT`, use the following pattern:
...
* Loads must target an empty table to meet this overlap rule.

* Later loads must use a key range that does not overlap existing data.

* Tables with secondary indexes must stay empty.

#### Visibility rules

* Rows remain invisible until MyRocks ingests the SST file.

* MyRocks ingests the current SST file when `rocksdb_bulk_load` is set to `OFF`.

* MyRocks ingests the current SST file when the session starts a bulk load on a different table.

* `SELECT` statements during bulk load return only older rows.

* Bulk-load sessions must not interleave `INSERT` statements across two or more tables.
...
* [MyRocks server variables](myrocks-server-variables.md#rocksdb_bulk_load)

* [MyRocks Information Schema tables](myrocks-information-schema-tables.md)

* [Extended mysqldump](extended-mysqldump.md#rocksdb-support)
