...
The `mysql` client uses a separate `telemetry_client` plugin. See [OpenTelemetry client plugin](opentelemetry-client.md).
...
!!! warning
    Uninstalling the component unloads it without a restart. Collection stops during the `UNINSTALL COMPONENT` statement. The component flushes traces and logs with a 10 ms timeout, then shuts the exporters down. Queued data may be lost if the collector is slow or unavailable. This action creates gaps in your monitoring dashboards. Before you remove the component, confirm that your observability platforms, such as Percona Monitoring and Management or an external OpenTelemetry collector, do not depend on this data.
...
This statement unloads the component from memory and also removes the component entry from the `mysql.component` table. After uninstallation, the component does not load during future server restarts. The statement does not clear persisted telemetry variables.
