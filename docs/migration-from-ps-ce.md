...
Percona Server and MySQL CE share core code. Percona adds features that MySQL CE does not include. A few of those features create a ["one-way door"](#summary-of-the-one-way-door-risk) because they change how data or metadata is written to disk, and MySQL CE cannot interpret the result. The one-way-door features are MyRocks, compressed columns, compression dictionaries, Percona keyring components (Vault, KMIP, AWS KMS), and Percona-only on-disk options such as enhanced encryption, specialized page compression, and `fast_checksum`.

The remaining Percona-only features do not store or alter any data. The Percona Audit Log plugin, the Audit Log Filter, Data Masking, PAM/LDAP authentication, and the thread pool are runtime features. Turn them off and remove their variables from `my.cnf` before you migrate. No data is lost or changed. Oracle plans to release the MySQL Enterprise equivalents of these features as open source, so you can optionally enable those on the CE instance after migration.
...
The following table lists migration actions for security and auditing features that are exclusive to Percona. None of these features change stored data, so turning them off does not lose or alter anything. See [Exclusive features](#exclusive-features) for the full list.
...
A MySQL CE 8.4 instance can replicate as a downstream replica from a Percona Server 8.4 source when the replica uses only CE-supported features. Confirm the following before cutover:

* Row-Based Replication (RBR) is recommended on the source and replica. Set `binlog_format=ROW`. The known statement-based incompatibility with MySQL CE is Percona's `SET STATEMENT <variable>=<value> FOR <statement>` syntax. RBR logs row changes instead of that statement, so the replica does not have to parse it. Statement-based or mixed replication can work when the binary log does not contain `SET STATEMENT`. Use the upstream-compatible `SET_VAR` optimizer hint instead; see [Extended SET VAR optimizer hint](extended-set-var.md).

* MyRocks does not have to be absent from the source. The binary log is engine-independent, so row events from MyRocks tables apply on the replica as ordinary row changes. Create the matching tables on the MySQL CE replica as InnoDB (for example, restore a dump after changing `ENGINE=MyRocks` to `ENGINE=InnoDB`). DDL that specifies `ENGINE=ROCKSDB` fails on CE because the engine is not present. Convert MyRocks tables to InnoDB on the source only when you also plan to run MySQL CE on that instance after cutover.

* Percona Toolkit UDFs (`fnv_64`, `fnv1a_64`, `murmur_hash`) do not have to be removed from the source. MySQL CE does not ship `component_percona_udf`, so a replica that executes those function names must define matching UDFs locally. Create them on the replica with `CREATE FUNCTION ... SONAME` (using the Toolkit UDF libraries). Alternatively, rewrite or drop dependent routines on the source so the replica never calls the functions. See [Percona Toolkit UDFs](udf-percona-toolkit.md).
...
Only the high-risk rows are true one-way doors. Those features change data or metadata on disk and require conversion before you migrate. The low-risk rows are runtime features or configuration entries: turn them off, and your data is unaffected.
...
| Audit Log Plugin / Audit Log Filter / Data Masking | Low | Uninstall the plugin or component and remove its configuration. No stored data is affected. The MySQL Enterprise equivalents, which Oracle plans to open source, can replace them after migration. |
| PAM / LDAP Authentication | Low | Reassign affected accounts to a CE-supported authentication plugin before the switch. No stored data is affected, but those accounts need new credentials. |
| Configuration | Low | Requires manual cleanup of `my.cnf`. |
| Thread Pool | Low | Remove `thread_handling=pool-of-threads` and the `thread_pool_*` variables to return to default thread handling. No stored data is affected. |
...
| Replication | Rolling migration (CE replica from Percona source) | RBR recommended; no `SET STATEMENT` in statement-based binlog; MyRocks DML is fine (create replica tables as InnoDB); Toolkit UDFs optional on replica (`CREATE FUNCTION`); matching GTID settings; CE-clean `my.cnf` on replica |
