...
Unlike plain [JSON Lines](https://jsonlines.org/), Percona JSONL keeps a wrapping JSON array and commas between lines. The file stays valid JSON and remains line-friendly for `grep`, `jq`, `wc -l`, streams, and aggregators.
...
* [Audit log filter functions, options, and variables](audit-log-filter-variables.md) — `audit_log_read()`, `audit_log_read_bookmark()`, and format options
