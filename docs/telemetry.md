...
!!! important "The two systems are independent"

    Installation-time telemetry and continuous telemetry are separate. Each system has its own opt-out method:

    * Installation-time telemetry is controlled by the `PERCONA_TELEMETRY_DISABLE` environment variable, set before you install a package or start a container.

    * Continuous telemetry is controlled by the server option `percona_telemetry_disable=1`, the `UNINSTALL COMPONENT` statement, and stopping the `percona-telemetry-agent` service.

    Disabling one system does not disable the other. To opt out of all telemetry on a host, follow both [Disable installation-time telemetry](#disable-installation-time-telemetry) and [Disable continuous telemetry](#disable-continuous-telemetry).
...
Installation-time telemetry is enabled by default. Set the environment variable `PERCONA_TELEMETRY_DISABLE=1` before you install packages or start a container.

This step disables installation-time telemetry only. Continuous telemetry keeps running on the installed server. To stop continuous uploads as well, also follow [Disable continuous telemetry](#disable-continuous-telemetry).
...
=== "Docker"
    ...
    ```shell
    docker run -d -e MYSQL_ROOT_PASSWORD=test1234# -e PERCONA_TELEMETRY_DISABLE=1 --name=percona-server percona/percona-server:{{vers}}
    ```
...
These steps disable continuous telemetry only. They do not affect installation-time telemetry, which has already run on hosts where the package or container is installed. To prevent installation-time telemetry on future installs or new containers, also follow [Disable installation-time telemetry](#disable-installation-time-telemetry).
