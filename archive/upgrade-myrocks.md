...
MySQL {{vers}} only provides support for partitioned table for the InnoDB storage engine.
...
Complete these steps for each table that **mysqlcheck** list. Otherwise, the upgrade to {{vers}} fails and your error log contains messages like the following:
