...
In *Percona Server for MySQL* Percona Server 5.7.20-18 has implemented TokuDB integration
with [Performance Schema](https://dev.mysql.com/doc/refman/5.7/en/innodb-performance-schema.html)
...
```sql
mysqlSELECT * FROM performance_schema.setup_instruments WHERE NAME LIKE "%/fti/%";
```
...
```sql
mysqlSELECT * FROM performance_schema.events_waits_summary_global_by_event_name WHERE EVENT_NAME LIKE '%fti%';
```
